...
// Serverless hosts (Vercel and most others) deploy your code to a
// read-only filesystem — the only writable location is the OS temp
// directory, and even that isn't guaranteed to persist between requests.
// We prefer a local ./data folder for normal local development (easy to
// open and inspect), but fall back automatically to the temp directory if
// the local folder can't be created or written to, so the app doesn't
// crash on a read-only host. See README -> "Moving to production" for why
// this fallback still isn't a substitute for a real database once you
// have real users.
const PROJECT_DATA_DIR = path.join(process.cwd(), "data");
const TMP_DATA_DIR = path.join(os.tmpdir(), "beowned-politics-data");
...
async function resolveDataDir() {
  ...
  try {
    ...
    const probe = path.join(PROJECT_DATA_DIR, ".write-check");
    await fs.writeFile(probe, "ok");
    await fs.unlink(probe);
    resolvedDataDir = PROJECT_DATA_DIR;
  } catch {
    await fs.mkdir(TMP_DATA_DIR, { recursive: true });
    resolvedDataDir = TMP_DATA_DIR;
  }
...
async function writeData(data) {
  ...
  writeQueue = writeQueue.then(() =>
    fs.writeFile(dbFile, JSON.stringify(data, null, 2))
  );
  await writeQueue;
}
