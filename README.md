...
Also fixed: **login/signup could crash on Vercel with a cryptic "Unexpected
end of JSON input" browser error.** The cause was that the data layer tried
to write to a folder inside the deployed project, which serverless hosts
like Vercel make read-only — the write failed, the API route crashed
before sending any response, and the browser saw an empty reply where it
expected JSON. Fixed two ways: (1) `lib/db.js` now detects when its normal
storage folder can't be written to and automatically falls back to the
OS temp directory, which serverless hosts do allow writing to; (2) every
API route now wraps its logic in a try/catch that always returns a proper
JSON error response, so any future unexpected failure shows a real message
instead of a blank response the browser can't parse.

This does **not** change the underlying persistence caveat below — the
temp directory isn't guaranteed to survive between requests either. It
just means the app degrades to "works for your current session, may lose
data between deploys" instead of "crashes outright."

**Login codes weren't reaching anyone on the live site.** The code was
only shown on-screen when `NODE_ENV !== "production"` — but Vercel always
runs in production mode, and there's no real email/SMS provider wired up
yet, so codes were generated but never delivered anywhere. Fixed: the code
is now shown by default (controlled by `SHOW_LOGIN_CODE`, not `NODE_ENV`),
with the login screen explaining plainly why it's visible. Once you wire
up a real email/SMS provider, set `SHOW_LOGIN_CODE=false` in your
environment to stop showing it.

**Signup was confusing** — there was no separate "create account" flow,
because signing in with a new email *is* the signup (an account is
created automatically on first login). That wasn't explained anywhere.
Fixed: the sign-in screen now says so directly ("Sign In / Sign Up... new
or returning... we'll create your account automatically").

**The app looked like a phone mockup even on desktop.** Every screen was
boxed inside a fixed-width, rounded, drop-shadowed "device" card
regardless of viewport, so a normal desktop browser saw a phone screenshot
floating in black space instead of a website. Fixed: below 769px wide, it
behaves like a mobile app (edge-to-edge, bottom tab bar). At 769px and
wider, it behaves like a normal responsive website (full-width page, a
real header with horizontal nav links, no bottom tabs, no floating
mockup card, comfortable centered reading column).
